...
PBC合宿2027（2027年4月27日（火）・身延山大学 本館5F講堂・246名を基準に会場レイアウトを最終調整中）の情報サイト。
...
### Codex更新運用メモ（2026-07-08追記）

- 白都さんからPBC合宿2027運営ハブ向けの新規情報・リンク・資料更新を受けた場合は、公開反映指示込みとして扱い、編集・チェック・commit・push・公開URL確認まで一連で進める。
- ただし、個人連絡先・全員名簿・金額原本・内部数値・権限変更・参加者向け公開可否に迷う情報は、従来どおり確認してから反映する。
- 運営ハブのリンク集にローカル参照（PC内パス、Dropbox内パス、`_work/` 参照など）は載せない。閲覧者が開ける公開URLまたは権限設定済みのGoogle Drive共有リンクに置き換える。

### 申込状況確認・督促運用メモ（2026-07-10追記）

- 申込状況DBと参加者リストの現在の入口は、Chatwork `拡大グループ` の概要欄を正本として確認する。
- 申込済み反映や未申込者へのLINE督促では、Google Sheets上の個人名を扱うため、個人名・行番号・具体的な突合結果は公開リポジトリ配下のHTMLやREADMEに載せない。
- 具体的なシートID、対象タブ、突合列、2026-07-10時点の反映済み行、未申込者候補、次回手順は、Git管理外の `HANDOVER.md` と `_work/registration-followup-20260710.md` を参照する。
- 2026-07-14の最新照合・反映結果は、Git管理外の `_work/registration-followup-20260714.md` を参照する。
- 2026-07-21の最新照合・反映結果とグループLINE督促案は、Git管理外の `_work/registration-followup-20260721.md` を参照する。
- 2026-07-29の最終照合・反映結果は、Git管理外の `_work/registration-followup-20260729.md` を参照する。
...
- 2027運営体制（7チーム。統括・会場は責任者決定済み）の残りのリーダー・人数確定後、`ops-jx9rmblg/teams/*.html` と `js/ops-config.js` を更新
- 会場使用の申込書類・正式見積・支払条件、当日詳細タイムテーブル、受付導線、持ち物が確定後、`ops-jx9rmblg/js/ops-config.js` の `todos` から該当項目を削除し、参加者ページ・運営ハブの該当箇所を更新（tbdバッジ・TODOコメントの解消）
- 新規申込のフォーム完了画面、LINE導線、申込完了メール、請求書、二重送信時の挙動は、実機確認後に確認済みとして記録する

## 更新履歴

- 2026-09-18: 身延山大学から2027年4月27日の会場使用承諾を受領。参加者向けページの会場表記を確定し、内部運用HPへ冒頭3〜5分の歓迎挨拶・紹介、学長への依頼・仮承諾、申込書類待ちを反映
- 2026-09-15: 講堂E案の246名を基準に会場レイアウトを最終調整する方針と、人数が多少変動する可能性を参加者ページ・運営ハブへ反映
- 2026-09-15: 新規参加者向けLPの公開を確認し、テーマ「価値あるゲーム」、受付10:30、11:00開始、18:00終了予定、一般申込締切、LP導線、LINE参加による申込確定の運用を参加者ページ・運営ハブへ反映
- 2026-09-15: 各チームの前年リーダーが同じチームのサブリーダーとして参加し、ノウハウを継承・蓄積しながらリーダーをローテーションする運営ルールを反映
- 2026-09-15: 身延山大学講堂E案・246名を基準に配置を最終調整。大学側の歓迎挨拶・紹介を冒頭3〜5分設けること、参加者への会場背景の事前案内、会場確約後に5分程度で時間調整する進行を参加者ページ・運営ハブへ反映
...
- 2026-07-14: 申込フォームと参加者リストを再照合し、申込済みの未反映分を `合宿登録完了` へ反映。詳細はGit管理外の `_work/registration-followup-20260714.md` に保存
- 2026-07-21: 浜松の申込状況を再照合し、申込済みの未反映分を `合宿登録完了` へ反映。未申込者向けグループLINE文案まで作成し、詳細はGit管理外の `_work/registration-followup-20260721.md` に保存
- 2026-07-22: 参加者リストに保留・申込予定の内部備考を追加し、未申込・要確認者向けグループLINEを送付済み。詳細はGit管理外の `_work/registration-followup-20260722.md` に保存
- 2026-07-29: 浜松の申込状況を最終照合し、申込済みの未反映分を `合宿登録完了` へ反映。参加対象者の申込完了を確認し、運営側への完了報告まで実施。詳細はGit管理外の `_work/registration-followup-20260729.md` に保存
